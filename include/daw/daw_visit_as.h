...
namespace daw {
	 ...
	 * allowing modification of the value via the provided visitor, and optionally
	 *        returning a value from the visitor's result.
	 *
	 * @tparam VisitedT The type to reinterpreted `value` as when invoking the
	 * visitor.
	 ...
	 *
	 * @param value A reference to the value being visited and potentially
	 * modified.
	 * @param visitor A invocable that processes the value once reinterpreted as
	 *                type `VisitedT`.
	 *
	 * @return The result of invoking the visitor if the visitor returns a
	 * non-void value; otherwise, nothing is returned.
	 *
	 * @details The function performs a bit cast to reinterpret `value` as type
	 *          `VisitedT`. It then applies the visitor to this reinterpreted
	 * value. After the visitor processes the value, the function updates the
	 ...
	 * @brief Invokes a visitor on a reinterpretation of the provided value.
	 *
	 * @tparam VisitedT The type to reinterpret `value` as when invoking the
	 * visitor.
	 * @tparam OrigT The original type of the `value`.
	 * @tparam Visitor The callable type that will be applied to the reinterpreted
	 * value.
	 *
	 * @param value A constant reference to the value being reinterpreted and
	 * visited.
	 ...
	 *
	 * @details This function performs a bit-cast of `value` to the type
	 * `VisitedT` and then applies the visitor to the reinterpreted value. The
	 * result of the visitor is returned if it produces a value.
	 */
	template<typename VisitedT, typename OrigT, typename Visitor>

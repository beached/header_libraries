...
namespace daw::from_string_trait {
	...
	struct from_string_t<I> {
		...
		DAW_CPP23_STATIC_CALL_OP constexpr std::optional<I>
		operator( )( daw::string_view str ) DAW_CPP23_STATIC_CALL_OP_CONST {
			I result = I{ };
			std::from_chars_result fs_r =
			  std::from_chars( str.data( ), str.data_end( ), result );
			if( fs_r.ec == std::errc( ) ) {
				return result;
			}
			return std::nullopt;
	...
	namespace impl {
		...
		concept StringConstructible =
		  requires( char const *p, std::size_t sz ) { String( p, sz ); };
	} // namespace impl
	...
	struct from_string_t<String> {
		...
		DAW_CPP23_STATIC_CALL_OP constexpr std::optional<String>
		operator( )( daw::string_view str ) DAW_CPP23_STATIC_CALL_OP_CONST {

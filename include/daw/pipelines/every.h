...
namespace daw::pipelines::pimpl {
	...
	struct every_iterator_end {
		...
		[[noreturn]] DAW_ATTRIB_NOINLINE every_iterator_end &operator++( ) const {
			std::terminate( );
		}

		[[noreturn]] DAW_ATTRIB_NOINLINE every_iterator_end
		operator++( int ) const {
			std::terminate( );
		}
	};
	...
	public:
		...
		explicit constexpr every_index_iterator(
		  ...
		  , m_every_nth( every_nth ) {
			...
			m_count = as<difference_type>( len / m_every_nth );
			if( len % m_every_nth != 0 ) {
				++m_count;
			}
		}

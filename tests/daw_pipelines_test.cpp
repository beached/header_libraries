...
namespace {
	...
	static_assert(
	  std::forward_iterator<filter_view<char const *, AlwaysTrue_t>> );
	...
	namespace tests {
		...
		void test002( ) {
			constexpr auto pm2 =
			  pipeline( Map( to_letter ), Enumerate, To<std::map> );
		...
		DAW_ATTRIB_NOINLINE void fma( std::size_t N,
			...
			static_assert(
			  std::same_as<daw::range_category_t<DAW_TYPEOF( fma_range )>,
